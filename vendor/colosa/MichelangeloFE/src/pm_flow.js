...
PMFlow.prototype.saveAndDestroy = function () {
    ...
    this.updateIncomingAndOutgoingElements("remove");
 ...
 * @returns {PMFlow}
 */
PMFlow.prototype.updateIncomingAndOutgoingElements = function(action){
    ...
    if (sourceShape && destinyShape){
        if (action === "remove"){
            sourceShape.removeOutgoingElement(destinyShape);
            destinyShape.removeIncomingElement(sourceShape);
        }
        if (action === "create"){
