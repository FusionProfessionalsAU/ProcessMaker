...
PMDesigner.taskProperties = function (activity) {
            ...
            onChange: function (val) {
                ...
                switch (val) {
                    ...
                    case 'FIELD' :
                        ...
                        break;
                    case 'CUSTOM' :
                        abeForm.setItems([abeFields.type, abeFields.emailFrom, abeFields.email, abeFields.subject, abeFields.template, abeFields.action, abeFields.note, abeFields.forceLogin]);
                        abeForm.getField("ABE_CASE_NOTE_IN_RESPONSE").setValue(abeFields.note.value);
                        abeForm.getField("ABE_FORCE_LOGIN").setValue(abeFields.forceLogin.value);
                        if(!abeAddOption){
    ...
    function loadTemplate(response) {
        var field = formNotifications.getField('tas_def_message_template'), i, field2;
        field.clearOptions();
        field.addOption({
